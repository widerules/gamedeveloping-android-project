...
public class PathFinder {
    ...
    public Direction getDirection(Trooper self, World world, int destX, int destY, double precision){
        ...
        System.out.println("finalNode: "+finalNode.x+" " +finalNode.y + ", opened: "+opened.size()+", closed: "+closed.size());
...
//            if ( (Math.abs(node.x - destX)+ Math.abs(node.y - destY)<precision)){
    ...
    public Node getNextReachable(TreeSet<Node> opened, LinkedList<Node> closed, Trooper self, CellType[][] cellTypes , int destX, int destY, TrooperStance stance, int depth, double precision){
        Node node = null;//new Node(self.getX(), self.getY(),10, null);
...
//            if ( (Math.abs(node.x - destX)+ Math.abs(node.y - destY)<precision)){
            if ( Math.sqrt((node.x - destX)*(node.x - destX) + (node.y - destY)*(node.y - destY))<=precision
                    &&
                    world.isVisible(self.getShootingRange(), node.x, node.y, self.getStance(), destX, destY, stance)){
                return node;
            }
    ...
    public Direction getDirectionReachable(Trooper self, World world, int destX, int destY, TrooperStance stance, double precision){
        TreeSet<Node> opened = new TreeSet<>();
        LinkedList<Node> closed = new LinkedList<>();
        ...
        System.out.println("reachable heading to: "+destX +" "+destY+ " from: "+self.getX() + " "+self.getY());

        Node finalNode = getNextReachable (opened, closed, self, cellTypes, destX ,destY,stance, 0, precision);
